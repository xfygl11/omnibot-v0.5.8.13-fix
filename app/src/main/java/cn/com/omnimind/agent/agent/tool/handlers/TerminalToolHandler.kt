...
import cn.com.omnimind.agent.agent.*
import cn.com.omnimind.agent.agent.AgentCallback
import cn.com.omnimind.agent.agent.AgentExecutionEnvironment
...
import cn.com.omnimind.agent.terminal.EmbeddedTerminalSessionRegistry
import cn.com.omnimind.agent.termux.TermuxCommandResult
import cn.com.omnimind.agent.termux.TermuxCommandSpec
import cn.com.omnimind.agent.termux.TermuxCommandRunner
...
class TerminalToolHandler(
    ...
    private fun buildSessionSetupSegments(workingDirectory: String?, environment: Map<String, String>): List<String> {
        ...
        environment.forEach { (rawKey, rawValue) ->
            ...
            segments += "export $key=${cn.com.omnimind.agent.termux.TermuxCommandBuilder.quoteForShell(rawValue)}"
        }
        if (!workingDirectory.isNullOrBlank()) {
            segments += "cd ${cn.com.omnimind.agent.termux.TermuxCommandBuilder.quoteForShell(workingDirectory)}"
        }
        return segments
    }
