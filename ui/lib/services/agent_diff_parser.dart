...
enum AgentDiffLineKind { header, add, remove, context, meta }
...
class AgentDiffSummary {
  ...
  final List<AgentDiffFile> files;
  final int additions;
  final int deletions;
...
AgentDiffSummary parseAgentDiffText(String diffText) {
  final normalized = _normalizeNewlines(diffText).trimRight();
  if (normalized.trim().isEmpty) {
    return const AgentDiffSummary(
      files: <AgentDiffFile>[],
      additions: 0,
      deletions: 0,
      sourceText: '',
...
String? extractAgentDiffPath(dynamic source) {
  return _extractFirstPathCandidate(source);
}

bool looksLikeAgentDiff(String value) {
...
String formatAgentDiffStat({required int additions, required int deletions}) {
  return '+${_compactCount(additions)} -${_compactCount(deletions)}';
}

String summarizeAgentDiff(AgentDiffSummary summary) {
...
String buildAgentUnifiedDiffFromPatch({
...
String buildAgentUnifiedDiffFromStrings({
...
class _UnifiedDiffParser {
  ...
  final List<AgentDiffFile> _files = <AgentDiffFile>[];
  ...
  int? _newLine;
  bool _inHunk = false;
  final List<AgentDiffLine> _lines = <AgentDiffLine>[];

  AgentDiffSummary parse() {
    ...
    return AgentDiffSummary(
      files: List<AgentDiffFile>.unmodifiable(_files),
      additions: additions,
      deletions: deletions,
      sourceText: diffText,
    );
  ...
  void _addMeta(String line) {
    _lines.add(
      AgentDiffLine(kind: AgentDiffLineKind.meta, content: line, prefix: ''),
    );
  }
  ...
  void _finishFile() {
    ...
    _files.add(
      AgentDiffFile(
        ...
        lines: List<AgentDiffLine>.unmodifiable(_lines),
