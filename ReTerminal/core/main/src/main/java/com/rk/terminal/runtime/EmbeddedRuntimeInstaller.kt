...
object EmbeddedRuntimeInstaller {
    ...
    private val UBUNTU_MIRRORS = listOf(
        // 1st priority: Tsinghua (verified reachable, content-length=29870567)
    ...
    /**
     * Tries each Ubuntu mirror URL in priority order. Returns the first manifest
     * entry whose download succeeds, or throws on the last failure.
     */
    @VisibleForTesting
    internal fun findUbuntuMirror(): RuntimeManifestEntry {
        val base = RuntimeManifestEntry(
            ...
            downloadUrl = requireHttpsUrl(UBUNTU_MIRRORS.last(), "Ubuntu 官方下载地址")
        )
        val tested = mutableSetOf<String>()
        for (url in UBUNTU_MIRRORS) {
            val httpUrl = url.trim().toHttpUrlOrNull() ?: continue
            if (!httpUrl.host.endsWith("ubuntu.com") &&
                !httpUrl.host.contains("tsinghua") &&
                !httpUrl.host.contains("aliyun") &&
                !httpUrl.host.contains("tencent") &&
                !httpUrl.host.contains("sjtu")) continue
            if (httpUrl.toString() in tested) continue
            tested.add(httpUrl.toString())
            // Lightweight connectivity probe: HEAD or short GET to verify URL reachable.
            val probeReq = Request.Builder().url(httpUrl).get().build()
            var probeOk = false
            try {
                client.newCall(probeReq).execute().use { resp ->
                    probeOk = resp.isSuccessful || resp.code in listOf(301, 302, 307, 308)
                }
            } catch (_: Exception) { probeOk = false }
            if (!probeOk) continue
            val entry = base.copy(downloadUrl = httpUrl)
            // Verify size hint matches before committing to full download.
            try {
                val headReq = Request.Builder().url(httpUrl).head().build()
                client.newCall(headReq).execute().use { resp ->
                    val contentLength = resp.header("Content-Length")?.toLongOrNull()
                    if (contentLength != null && contentLength != base.compressedSize.toLong()) {
                        probeOk = false
                    }
                }
            } catch (_: Exception) { /* ignore head failures */ }
            if (probeOk) return entry
        }
        // Fallback to official URL if no mirror responded.
        return base
    }
    ...
    private suspend fun downloadVerifiedArchive(
        ...
        // Build mirror list: start with the resolved entry URL, then add any
        // remaining UBUNTU_MIRRORS URLs that differ from it.
        val candidateUrls = buildList {
            add(entry.downloadUrl.toString())
            for (mirrorUrl in UBUNTU_MIRRORS) {
                val u = mirrorUrl.trim().toHttpUrlOrNull() ?: continue
                if (u.toString() != entry.downloadUrl.toString()) add(u.toString())
            }
        }.distinct()

        for (mirrorIdx in candidateUrls.indices) {
            val mirrorUrl = requireHttpsUrl(candidateUrls[mirrorIdx], "Ubuntu 下载地址")
            val mirrorEntry = entry.copy(downloadUrl = mirrorUrl)
            for (attempt in 0 until 2) {
                try {
                    downloadAttempt(distribution, mirrorEntry, partial, onProgress)
                    if (partial.length() == mirrorEntry.compressedSize &&
                        sha256OrNull(partial) == mirrorEntry.sha256) {
                        replaceWithFile(archive, partial)
                        context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE).edit()
                            .putString("${distribution.id}_version", mirrorEntry.version)
                            .putString("${distribution.id}_sha256", mirrorEntry.sha256)
                            .putLong("${distribution.id}_expanded_size", mirrorEntry.expandedSize)
                            .apply()
                        return
                    }
                    partial.delete()
                    if (attempt == 0) {
                        val srcName = if (mirrorIdx > 0) "镜像 ${mirrorIdx + 1}" else "官方"
                        emit(onProgress, "verifying", distribution,
                            "${srcName} 校验失败，正在重新完整下载")
                    }
                } catch (e: Exception) {
                    if (attempt == 0) {
                        val srcName = if (mirrorIdx > 0) "镜像 ${mirrorIdx + 1}" else "官方"
                        emit(onProgress, "error", distribution,
                            "${srcName} 下载失败：${e.message ?: "未知错误"}")
                    }
                    partial.delete()
                }
            }
            if (mirrorIdx < candidateUrls.size - 1) {
                emit(onProgress, "status", distribution,
                    "当前源不可用，切换到下一个镜像源重试…")
            }
        }
